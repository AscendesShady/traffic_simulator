...
> **Staleness note:** this file embeds full source snapshots that go stale the moment the real source changes, and re-syncing a multi-thousand-line dump for every change is not a practical maintenance model. As of the session that fixed the DBL stuck-bus bug, added the "AI Decision Audit" State→Action+Reasoning→Outcome sheet, added `bus_distribution` telemetry, and moved the calculated green-time display into `telemetry_dashboard.py`, the embedded dumps of `vehicle.py`, `signal_controller.py`, `agent.py`, `rule_controller.py`, `telemetry_exporter.py`, `telemetry_dashboard.py`, `control_panel.py`, and `main.py` below predate those changes (this file also already lagged behind earlier, unrelated changes — e.g. the `queue_length_m`/`downstream_space_m` telemetry fields are missing from the embedded dump). **For current behavior, trust the actual source files, `CLAUDE.md`, and `context.md` over the code blocks in this document**; treat this guide's prose sections (architecture, safety contracts, responsibilities) as the durable part and its embedded code as a point-in-time snapshot only.
...
- A DBL request/grant is vetoed whenever a stopped or crawling vehicle sits ahead of the bus in the reserved lane (`vehicle.dbl_lane_queue_ahead`/`dbl_lane_is_obstructed`), even once the bus is already in that lane; clearing a non-bus vehicle out of the reserved lane is unconditional across the whole approach, not gated by its distance from the bus, and a vetoed leg abandons the merge immediately and stickily rather than waiting out the usual bounded-abandon timer.
...
- The "AI Decision Audit" Excel sheet (`main.write_ai_decision_audit_sheet`) adds one row per logged decision with the exact telemetry state observed at that decision (reasoning, requested/observed TSP and DBL routes, queues, network summary, bus distribution) plus an outcome snapshot of the state at the following decision (or the nearest later telemetry sample for the final decision), so a grant can be judged after the fact without new instrumentation. An export meant to show only live/in-memory state must pass `decisions`/`telemetry_rows` explicitly (even as empty lists) rather than relying on a default, since a default that silently reads the on-disk session logs would leak stale decisions from an earlier run into a live-only export.
- `traffic_state_telemetry.json` includes a `bus_distribution` block (`TelemetryExporter.compute_bus_distribution`): total bus count, buses by node/approach segment, buses by route, and buses currently in the DBL lane, all read at the same timestamp/frame as the rest of the payload.
- The telemetry dashboard's Summary tab owns the entire Webster timing readout: a WEBSTER SIGNAL TIMING card with measured lane capacity and each node's condition, cycle length and demand ratio (plus an oversaturation note), and a green-time label under each node's signal-color indicator. The control panel no longer renders any of it — it holds operator inputs, while derived timing is observed output shown beside the live signal state. `control_panel.get_webster_timing_summary()` remains the single source both read.
- The Batch Run card has its own decision-interval slider (`batch_runtime["tick_seconds"]`), independent of the Single Run card's (`ai_runtime["tick_seconds"]`). `control_panel._effective_tick_seconds()` resolves which one `ai_control.json` receives — the batch value while `test_running` is true (a Benchmark Test or any queued Batch Benchmark run), the Single Run value otherwise — so neither setting silently governs or overwrites the other.
