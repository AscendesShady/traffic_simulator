...
| **LLM, assisted** (any Ollama tag or Gemini model) | A separate-process LangGraph loop reads the telemetry snapshot every tick (2–120 s, default 10 s), renders a structured text "minimap" (per-node competing queues in passengers, per-route bus positions/ETAs/loads, receiving-lane state, lane obstruction) and asks the model for `{reason, tsp[6], dbl[6]}` with a prompt that states the objective (fewest person-hours for everyone), what each knob does and costs, and two independent tests: TSP when the bus would stop, is actionable after the expected latency and carries more passengers than the cross queue it would hold (one route per node); DBL on every route whose reserved lane is clear, never weighed against the cross street. It asks for a one-pass answer (DECIDE FAST). Until 2026-09-25 the objective sentence, a TSP-only worked example and a reason field naming only the passenger comparison led every model but grok-4 to skip DBL on nearly every bus-turn its own test granted (Gemini: 1 of 230, seed 987). A server-side `anti_cheat` step forces `dbl=False` for any new grant where telemetry shows the lane obstructed. Ollama calls time out at 45 s, Gemini at 30 s (temperature 0.2); a time-out is held all-off, and grid points that come due while the abandoned call still holds the provider are skipped (`SKIPPED_SLOW`), not held. |
